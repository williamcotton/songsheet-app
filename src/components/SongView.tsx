...
export function SongView({ rawText, songId, scrollContainerRef, extraControls }: SongViewProps) {
  const lastGoodSongRef = useRef<ReturnType<typeof parse> | null>(null);

  const parsedSong = useMemo(() => {
    try {
      const song = parse(rawText);
      lastGoodSongRef.current = song;
      return song;
    } catch {
      return lastGoodSongRef.current;
    }
  }, [rawText]);
  ...
  const { scrollTo } = useAutoScroll({ isScrolling: audio.isPlaying, containerRef: scrollContainerRef });

  const currentSong = useMemo(() => {
    if (!parsedSong) return null;
    if (semitoneOffset === 0) return parsedSong;
    return transpose(parsedSong, semitoneOffset);
  }, [parsedSong, semitoneOffset]);

  // Reset on song navigation
  useEffect(() => {
    setSemitoneOffset(0);
  ...
  useEffect(() => {
    ...
    const sectionEl = root.querySelector(`.section[data-structure-index="${structureIndex}"]`);
    if (!sectionEl) return;
    ...
    let target = (scrollEl as HTMLElement).offsetTop - controlsHeight - viewportHeight * 0.3;
  ...
  }, [audio.activeHighlight, audio.isPlaying, scrollTo, scrollContainerRef]);

  function applyTranspose(delta: number) {
    if (!parsedSong) return;
    const newOffset = semitoneOffset + delta;
    setSemitoneOffset(newOffset);
    const newSong = newOffset === 0 ? parsedSong : transpose(parsedSong, newOffset);
